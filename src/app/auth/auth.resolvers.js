...
const resolvers = {
  ...
  Mutation: {
    ...
    async checkCode(parent, { code }, { knex }) {
      try {
        ...
        console.error("Error occurred while checking code:", error);
        return false; // Return false in case of errors
      }
    },

    async resetPassword(parent, { email, newPassword }, { knex }) {
      ...
      await userQueries.update(knex, user.id, { password: hashedPassword });
