...
let Profile = {
    ...
    return new Promise(function (resolve, reject) {
         db.query("DELETE FROM disabilities_to_users WHERE user_id = ? ", [idUtente], function (err, result) {
            if (err) {
                return reject(err);
            }
            resolve(result);
        });
         db.query("DELETE FROM facilities_to_users WHERE user_id = ?; ", [idUtente], function (err, result) {
            if (err) {
                return reject(err);
            }
            resolve(result);
        });
        db.query("DELETE FROM `users` WHERE `id` = ? ", [idUtente], function (err, result) {
            if (err) {
                return reject(err);
            }
            resolve(result);
